...
void updateDoctorDatabase(string doctorId, std::string& fieldToUpdate, std::string& fieldValue) {
    ...
    if (fieldToUpdate == "doctorName" || fieldToUpdate == "practiceKeywords" || \
        ...
        string queryRating = "SELECT rating, ratingSubmissions FROM doctorInfo WHERE id = " + doctorId + ";";
        int rc = sqlite3_prepare_v2(db, queryRating.c_str(), -1, &stmt, 0);
        if (rc == SQLITE_OK) {
            rc = sqlite3_step(stmt);
            if (rc == SQLITE_ROW) {

                double currentRating = sqlite3_column_double(stmt, 0);
                int ratingSubmissions = sqlite3_column_int(stmt, 1);
                double newRating = (currentRating * ratingSubmissions + stod(fieldValue)) / (ratingSubmissions + 1);
                ratingSubmissions++;
                string updateRatingQuery = "UPDATE doctorInfo SET rating = " + to_string(newRating) + ", ratingSubmissions = " + to_string(ratingSubmissions) + " WHERE id = " + doctorId + ";";
                int exec2 = sqlite3_exec(db, updateRatingQuery.c_str(), NULL, NULL, &error);
